terraform {
  ...
  # replace with real backend resources before first init.
  backend "s3" {
    bucket         = "REPLACE_ME-terraform-state-prod"
    key            = "hotel-bookings/prod/terraform.tfstate"
    region         = "ap-south-1"
    dynamodb_table = "REPLACE_ME-terraform-locks-prod"
    encrypt        = true
  }
}
...
module "rds" {
  ...
  multi_az                = true # prod: multi-AZ for failover
  backup_retention_period = var.db_backup_retention_days
  deletion_protection     = var.db_deletion_protection
  skip_final_snapshot     = false # prod: always keep a final snapshot
